...
fun AppNavHost(
    ...
    NavHost(
        ...
        composable("active_rentals") {
            ActiveRentalsScreen(navController = navController, currentUserEmail = userEmail)
        }
    }
}
...
fun ItemDetailScreen(navController: NavHostController, itemId: String, rentalItems: List<RentalItem>, currentUserEmail: String) {
    ...
    LaunchedEffect(currentUserEmail) {
        val userKey = currentUserEmail.replace(".", "_").replace("@", "_")
        database.getReference("users").child(userKey).get()
            .addOnSuccessListener { snapshot ->
                val firstName = snapshot.child("firstName").getValue(String::class.java)
                userName = firstName ?: currentUserEmail.substringBefore('@')
                // Debug log
                android.util.Log.d("ItemDetailScreen", "UserName fetched: $userName")
            }
            .addOnFailureListener {
                userName = currentUserEmail.substringBefore('@')
                android.util.Log.e("ItemDetailScreen", "Failed to fetch user name")
            }
    }

    // Fetch the status of any existing rental request
    ...
    if (showRentDialog) {
        AlertDialog(
            ...
            confirmButton = {
                Button(
                    onClick = {
                        android.util.Log.d("ItemDetailScreen", "Duration: $rentalDuration, UserName: $userName")

                        when {
                            rentalDuration.isBlank() -> {
                                Toast.makeText(context, "Please enter a duration.", Toast.LENGTH_SHORT).show()
                            }
                            userName.isBlank() -> {
                                Toast.makeText(context, "Loading user info, please wait...", Toast.LENGTH_SHORT).show()
                            }
                            else -> {
                    ...
                    enabled = rentalDuration.isNotBlank() && userName.isNotBlank()
                ) {
                    Text("Send Request")
                }
            },
            ...
            LazyColumn(
                ...
                item {
                    ...
                    Button(
                        ...
                        Spacer(modifier = Modifier.width(8.dp))
                        Text("List  for Rent", fontSize = 16.sp)
                    }
